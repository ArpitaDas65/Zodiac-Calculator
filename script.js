...
function generateMessage() {
    ...
    if (isValidDate(day, month, year)) {
        ...
        const complimentIndex = getIndex(day * name.length, compliments.length);
        const victimIndex = getIndex((name.length + surname.length / 2) * month, victimCompliments.length);
        const recoIndex = getIndex((year * surname.length + (name.length / 2)) - day, recommendations.length);

        output.innerText = 
        `Hey ${name}, your Zodiac Sign is: ${zodiacSign}.  
${compliments[complimentIndex]}  
${victimCompliments[victimIndex]}  
${recommendations[recoIndex]}`;
    } 
    else {
        output.innerText = "Invalid date. Please enter a valid date.";
    }
}